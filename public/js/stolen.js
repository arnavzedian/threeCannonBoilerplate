...
function Environment() {
  ...
  var platformImage = "./textures/ground.jpg";
...
///---Buses---///
function Bus(platformSide) {
  //platformSide should be "platformLeft" or "platformRight"
  ...
  ///frame
  var bfp =
    bus.platformSide == "platformLeft"
      ? { x: -40, y: 3, z: 0 }
      : { x: 40, y: 3, z: 0 }; //bus frame position
  ...
  bus.interior = new Physijs.BoxMesh(
    busInteriorGeometry,
    busInteriorMaterial,
    5000
  );
  ...
  //   loader.load(`./gltf/bus_body_${color}.glb`, function (gltf) {
  //     var scale = 5.6;
  ...
  if (bus.platformSide === "platformLeft") {
    bus.frame.rotation.y = Math.PI;
  }
  ...
  var busWheelImage = "./textures/ground.jpg";
  var busWheelGeometry = new THREE.CylinderGeometry(fr, br, wi, segments);

  //wheel side & back material (color only, no image)
  var busWheelColorBaseMaterial = new THREE.MeshLambertMaterial({
    color: 0x000000,
  });
  var busWheelColorMaterial = Physijs.createMaterial(
    busWheelColorBaseMaterial,
    bwf,
    bwr
  );
  ...
  bus.wheel_fl = new Physijs.CylinderMesh(
    busWheelGeometry,
    busWheelMaterialsArray,
    300
  );
  bus.wheel_fr = new Physijs.CylinderMesh(
    busWheelGeometry,
    busWheelMaterialsArray,
    300
  );
  bus.wheel_bl = new Physijs.CylinderMesh(
    busWheelGeometry,
    busWheelMaterialsArray,
    300
  );
  bus.wheel_br = new Physijs.CylinderMesh(
    busWheelGeometry,
    busWheelMaterialsArray,
    300
  );
...
function aBusHasFallen() {
  return (
    busArray[0].frame.position.y < -50 || busArray[1].frame.position.y < -50
  );
}
...
function handleKeyDown(keyEvent) {
  ...
  switch (keyEvent.keyCode) {
    ...
    case 38: // "w" key or up arrow key (forward)
      busArray[0].wheel_bl_constraint.configureAngularMotor(
        2,
        1,
        0,
        -30,
      ...
      busArray[0].wheel_br_constraint.configureAngularMotor(
        2,
        1,
        0,
        -30,
    ...
    case 76: // "l" key (turn left)
      busArray[1].wheel_fr_constraint.configureAngularMotor(
        1,
        -Math.PI / 4,
        Math.PI / 4,
        10,
      ...
      busArray[1].wheel_fl_constraint.configureAngularMotor(
        1,
        -Math.PI / 4,
        Math.PI / 4,
        10,
